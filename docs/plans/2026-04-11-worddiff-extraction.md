...
- `(d *Differ) changedRanges(minus, plus string) ([]Range, []Range)` — called from `ComputeIntraRanges`
- `(d *Differ) tokenizeLineWithOffsets(line string) []intralineToken` — called from `changedRanges`, `passesSimilarityGateFromKeep`
- `(d *Differ) lcsKeptTokens(minus, plus []intralineToken) ([]bool, []bool)` — called from `changedRanges`, `passesSimilarityGateFromKeep`
- `(d *Differ) buildChangedRanges(tokens []intralineToken, keep []bool) []Range` — called from `changedRanges`
...
**`ComputeIntraRanges` as primary entry point**: combines tokenize → LCS → ranges → similarity gate in one pass. Returns nil ranges when similarity fails. The internal `changedRanges` (private method, no similarity gate) is called from `ComputeIntraRanges` and testable via same-package tests.
...
- [ ] create `app/ui/worddiff/worddiff.go` with package doc comment describing the sub-package
- [ ] define `Differ` struct (empty — stateless, exists for method namespace and interface wrapping) with godoc noting the pattern matches `style.SGR`
- [ ] implement `New() *Differ` constructor — returns `&Differ{}`
- [ ] define `Range` struct with exported `Start`, `End int` fields and godoc
- [ ] define `MaxLineLenForDiff` and `SimilarityThreshold` exported constants
- [ ] port `tokenPattern` regex var, `intralineToken` type (private data struct)
- [ ] port as private methods on `*Differ` (receiver unused, structural consistency per CLAUDE.md rule): `tokenizeLineWithOffsets`, `lcsKeptTokens`, `isWhitespaceToken`, `countNonWhitespace`, `buildChangedRanges` (change return type from `[]matchRange` to `[]Range`, `.start`/`.end` to `.Start`/`.End`), `passesSimilarityGateFromKeep`
- [ ] implement private method `(d *Differ) changedRanges(minus, plus string) ([]Range, []Range)` — ports `changedTokenRanges` logic with new type names
- [ ] implement `(d *Differ) ComputeIntraRanges(minusLine, plusLine string) ([]Range, []Range)` — method on Differ, combines tokenize + LCS + ranges + similarity gate in one pass. Returns nil ranges when similarity fails. This is the primary entry point called via the `wordDiffer` interface.
...
- [ ] define `LinePair` struct with exported `Content string`, `IsRemove bool` fields and godoc
- [ ] define `Pair` struct with exported `RemoveIdx`, `AddIdx int` fields and godoc
- [ ] implement `(d *Differ) PairLines(lines []LinePair) []Pair` — method on Differ. Partitions input into remove/add index lists, uses equal-count 1:1 or greedy path. Indices are into the input `lines` slice.
- [ ] port `greedyPairLines` as private method `(d *Differ) greedyPair(lines []LinePair, removes, adds []int) []Pair` — called only from `PairLines`, must be method per rule
- [ ] port `commonPrefixLen`, `commonSuffixLen` as private methods on `*Differ` — called only from `greedyPair`
...
- [ ] create `app/ui/worddiff/highlight.go` with package header
- [ ] implement `(d *Differ) InsertHighlightMarkers(s string, matches []Range, hlOn, hlOff string) string` — method on Differ, port from `diffview.go:537-594`. Remove Model receiver. Change `matchRange` → `Range`, `.start`/`.end` → `.Start`/`.End`. Call `d.updateRestoreBg` (private method).
- [ ] port `updateRestoreBg` as private method `(d *Differ) updateRestoreBg(seq, current, hlOff string) string` — called only from `InsertHighlightMarkers`, must be method per rule
...
- [ ] create `worddiff_test.go` with package `worddiff` header
- [ ] port `TestTokenizeLineWithOffsets` — same test, call `tokenizeLineWithOffsets` directly
- [ ] port `TestLCSKeptTokens` — same test
- [ ] port `TestBuildChangedRanges` — change `matchRange` → `Range` in expected values
- [ ] port `TestChangedTokenRanges` as `TestChangedRanges` — change function name and types
- [ ] port `TestChangedTokenRanges_MultibytePrecision` as `TestChangedRanges_MultibytePrecision`
- [ ] port `TestChangedTokenRanges_SkipsVeryLongLines` as `TestChangedRanges_SkipsVeryLongLines`
- [ ] port `TestPassesSimilarityGateFromKeep` — adapt to test internal `passesSimilarityGateFromKeep` directly
- [ ] port `TestPassesSimilarityGateFromKeep_WhitespaceOnly`
- [ ] add `TestComputeIntraRanges` — verify that similarity gate is applied: similar pair returns ranges, dissimilar pair returns nil
- [ ] port `TestCommonPrefixLen`, `TestCommonSuffixLen`
- [ ] port `TestModel_PairHunkLines` as `TestPairLines` — construct `[]LinePair` input instead of `testModel` with diffLines. Adapt all table cases.
- [ ] port `TestModel_PairHunkLines_BestMatchScoring` as `TestPairLines_BestMatchScoring`
- [ ] create `highlight_test.go` with package `worddiff` header
- [ ] add `TestInsertHighlightMarkers` — port relevant assertions from `search_test.go` that exercise the marker insertion (match at start, middle, end, overlapping ANSI, no matches, empty input)
- [ ] add `TestInsertHighlightMarkers_BgStateTracking` — test that bg-changing ANSI sequences inside a match cause hlOn re-emission and match-end restores to the correct bg
- [ ] add `TestUpdateRestoreBg` — table-driven: 24-bit bg, basic bg, reverse video, bg reset, full reset, non-bg sequence

### Task 5: M1 MILESTONE — Worddiff package green

This is the milestone gate for M1. No new code, just verification.

- [ ] run `go test ./app/ui/worddiff/... -race` — must be green
- [ ] run `go vet ./app/ui/worddiff/...` — must be clean
- [ ] run `golangci-lint run app/ui/worddiff/...` — must be clean
- [ ] verify test coverage: `go test ./app/ui/worddiff/... -cover` — target ≥ 80%
- [ ] acknowledge that full project build is still broken — expected
- [ ] commit M1: `feat(worddiff): add intra-line diff and highlight sub-package`
...
- [ ] add `import "github.com/umputun/revdiff/app/ui/worddiff"` to `model.go`
- [ ] add `wordDiffer` interface to `model.go` (alongside existing `styleResolver`, `styleRenderer`, `sgrProcessor`, `FileTreeComponent`, `TOCComponent` interfaces): 3 methods — `ComputeIntraRanges`, `PairLines`, `InsertHighlightMarkers`
- [ ] add compile-time assertion: `var _ wordDiffer = (*worddiff.Differ)(nil)`
- [ ] add `differ wordDiffer` field to `Model` struct
- [ ] add `WordDiffer wordDiffer` field to `ModelConfig` with godoc noting it's required
- [ ] update `NewModel` to validate `cfg.WordDiffer != nil` (return error) and copy into `m.differ`
- [ ] change `model.go:177` — `intraRanges [][]matchRange` → `intraRanges [][]worddiff.Range`
- [ ] add `import "github.com/umputun/revdiff/app/ui/worddiff"` to `diffview.go`
- [ ] delete `matchRange` type definition at `diffview.go:17`
- [ ] in `applyIntraLineHighlight`: replace `m.insertHighlightMarkers(textContent, ranges, hlOn, hlOff)` → `m.differ.InsertHighlightMarkers(textContent, ranges, hlOn, hlOff)`
- [ ] in `highlightSearchMatches` (diffview.go:486): change local `var matches []matchRange` → `var matches []worddiff.Range`
- [ ] in `highlightSearchMatches`: change `matches = append(matches, matchRange{start, start + len(term)})` → `matches = append(matches, worddiff.Range{Start: start, End: start + len(term)})`
- [ ] in `highlightSearchMatches`: replace `m.insertHighlightMarkers(s, matches, hlOn, hlOff)` → `m.differ.InsertHighlightMarkers(s, matches, hlOn, hlOff)`
- [ ] delete `insertHighlightMarkers` method (diffview.go:537-594)
- [ ] delete `updateRestoreBg` method (diffview.go:598-614)
- [ ] delete `TestModel_UpdateRestoreBg` from `diffview_test.go:937` — coverage moves to `highlight_test.go:TestUpdateRestoreBg`
- [ ] migrate `collapsed.go:295` — `buildModifiedSet` calls `m.pairHunkLines(start, end)`. Replace with: build `[]worddiff.LinePair` from `m.diffLines[start:end]`, call `m.differ.PairLines(block)`, check `len(pairs) > 0` (only the pair count matters here, indices are not used)
- [ ] add `import "github.com/umputun/revdiff/app/ui/worddiff"` to `collapsed.go`
- [ ] wire in `app/main.go`: add `import "github.com/umputun/revdiff/app/ui/worddiff"`, set `WordDiffer: worddiff.New()` in the `ModelConfig{...}` literal
...
- [ ] add `import "github.com/umputun/revdiff/app/ui/worddiff"` to `loaders.go` (for `worddiff.LinePair` and `worddiff.Range` type names)
- [ ] move `recomputeIntraRanges` method from `app/ui/worddiff.go` to `app/ui/loaders.go`
- [ ] rewrite the method body to: build `[]worddiff.LinePair` from `m.diffLines` block, call `m.differ.PairLines`, call `m.differ.ComputeIntraRanges` for each pair, store results in `m.intraRanges` (see Technical Details for the exact code)
- [ ] verify the method is called from `loaders.go:159` (handleFileLoaded) and `model.go:629` (toggleWordDiff) — both call sites unchanged since the method signature is identical
...
- [ ] delete `app/ui/worddiff.go` — all functions either moved to worddiff package or to loaders.go
- [ ] move `TestModel_RecomputeIntraRanges*` tests from `app/ui/worddiff_test.go` into `app/ui/loaders_test.go` — follows "one test file per source file" convention since `recomputeIntraRanges` now lives in `loaders.go`. Combined size: ~824 + ~118 = ~942 lines, within the ~1000 soft target.
- [ ] delete `app/ui/worddiff_test.go` — all tests either ported to `worddiff/worddiff_test.go` (pure functions) or merged into `loaders_test.go` (Model method tests)
- [ ] update `TestModel_RecomputeIntraRanges*` tests in `loaders_test.go`: change `matchRange` → `worddiff.Range`, update field access if needed
- [ ] update `diffview_test.go:742`: change `matchRange{0, 3}` → `worddiff.Range{Start: 0, End: 3}`
- [ ] update `diffview_test.go` imports to include `worddiff`
- [ ] update test helper (`testModel` or `testNewModel` or `ModelConfig` literals in test files) to include `WordDiffer: worddiff.New()` — every `ModelConfig{...}` in tests needs this field or `NewModel` returns error
- [ ] confirm no ui file still references `matchRange` (type), `changedTokenRanges` (function), `pairHunkLines` (method), `greedyPairLines` (method), `insertHighlightMarkers` (method), `updateRestoreBg` (method): `grep -rn 'matchRange\|changedTokenRanges\|pairHunkLines\|greedyPairLines\|insertHighlightMarkers\|updateRestoreBg' app/ui/*.go` — should find zero hits
- [ ] `go build ./...` — should compile now
...
- [ ] run `go test ./app/ui/... -cover` and verify coverage has not regressed
- [ ] build the binary: `make build`
- [ ] manual smoke test: launch `.bin/revdiff` on a git repo with word-diff enabled (`W` key) — verify intra-line highlighting works on add/remove lines
- [ ] manual smoke test: search (`/`) for a term — verify search highlighting works
- [ ] manual smoke test: `--no-colors` mode — verify reverse-video fallback works for both word-diff and search
- [ ] commit M2: `refactor(ui): extract worddiff sub-package`
...
- [ ] read `app/ui/doc.go` to understand its current structure
- [ ] update `app/ui/doc.go`: remove `worddiff.go` description, add `worddiff/` sub-package description noting it contains intra-line word-diff algorithms and the shared highlight marker insertion engine
- [ ] update `CLAUDE.md` Project Structure section: add `app/ui/worddiff/` entry describing the sub-package
- [ ] update `CLAUDE.md` Data Flow section: update the word-diff paragraph to note that `worddiff.ComputeIntraRanges()` and `worddiff.PairLines()` are in the sub-package, and `worddiff.InsertHighlightMarkers()` is used by both word-diff and search highlighting

### Task 11: M3 MILESTONE — docs complete, plan moved

- [ ] run `go test ./... -race` — must pass
- [ ] run `golangci-lint run --max-issues-per-linter=0 --max-same-issues=0` — must be clean
- [ ] verify `app/ui/worddiff/` test coverage ≥ 80%
- [ ] move this plan: `git mv docs/plans/2026-04-11-worddiff-extraction.md docs/plans/completed/`
- [ ] commit M3: `docs: complete worddiff extraction plan`
